...
**Option 3 — Deployment Script:** The `Deploy Proximos Settings.sh` script from **Settings → Extras → Export MDM Scripts** creates and deploys the settings plist at `/Library/Preferences/com.enlazo.proximos.settings.plist`. Customize the script with your organization's values before deploying. Because the file lands in `/Library/Preferences/`, a local administrator can delete it. Re-run the script on every MDM check-in to ensure it stays present.
