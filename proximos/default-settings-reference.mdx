...
One day is deliberately minimal. The grace period is not intended as an extended exemption from updates; it is a brief courtesy window. Most organizations deploying Proximos want enforcement to begin promptly. Adjust this value to match your onboarding timeline — common choices are `3 to 7` days .

> **Note:** The grace period clock counts from Proximos' first launch, not from MDM enrollment.
...
No custom engine URL is set. This should only be changed if directed by Enlazo Systems support.
